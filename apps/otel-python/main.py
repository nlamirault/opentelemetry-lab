...
LoggingInstrumentor(logging_format="%(asctime)s %(levelname)s [%(name)s] [%(filename)s:%(lineno)d] [trace_id=%(otelTraceID)s span_id=%(otelSpanID)s resource.service.name=%(otelServiceName)s trace_sampled=%(otelTraceSampled)s] - %(message)s")
...
async def chain(response: Response):
    ...
    async with httpx.AsyncClient() as client:
        logger.info(f"Call http://{TARGET_ONE_SVC}")
        await client.get(
            f"http://{TARGET_ONE_SVC}/",
        )
    async with httpx.AsyncClient() as client:
        logger.info(f"Call http://{TARGET_TWO_SVC}")
        await client.get(
            f"http://{TARGET_TWO_SVC}/",
        )
